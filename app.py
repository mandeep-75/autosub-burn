...
def get_best_ffmpeg_encoder():
    """Select the most efficient video encoder for the current system."""
...
with st.sidebar:
    ...
    # Optional: let user choose device
    ...
    if st.button("✨ Step 1: Generate Subtitles", type="primary"):
        with st.spinner(f"Transcribing with {model_size} model... (Using faster-whisper)"):
            try:
                ...
                # 2. Choose device and compute type
                ...
                # 3. Load model (automatically downloaded if not in cache)
                ...
                segments_generator, info = model.transcribe(
                    ...
                    initial_prompt=None,
                    vad_filter=False,                # optional: set True to filter non‑speech
                    vad_parameters=None
                )

                # 5. Convert to our internal format (word‑level segments)
                word_level_segments = []
                for segment in segments_generator:
                    if segment.words:
                        ...
                        # Fallback if word timestamps are missing (should not happen)
                ...
                # 6. Merge word segments into display lines
